...
foreach ($port in $ports) {
    ...
    echo "" | CloudflareST.exe -n 1000 -tp $port -dn 10  -p 10  -f "$PSScriptRoot\ipv4.txt" -o "$PSScriptRoot\result_$port.csv" -url $cloudflareSTUrl
    
    # 等待命令执行完毕，确保结果已写入文件
    Start-Sleep -Seconds 2
    
    # 读取并解析对应端口号的 result.csv 文件
    $csvContent = Get-Content -Path "$PSScriptRoot\result_$port.csv"
    ...
    $csvContent | Select-Object -Skip 1 | ForEach-Object {
        ...
        # 检查条件 延迟 必须小于 1000 且 下载速度 必须大于 1，否则跳过这行
        if ($columns[4] -ge 1000 -or $columns[5] -le 1) {
            continue
        }
